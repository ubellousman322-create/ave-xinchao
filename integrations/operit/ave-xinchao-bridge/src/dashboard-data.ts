...
import { ensureXinchaoService } from "./xinchao-service";
...
export interface XinchaoState {
  ...
  sessionOverlays?: Record<string, {
    sessionId?: string;
    updatedAt?: string;
    lastConversationAt?: string;
  }>;
  recentConversationEvents?: InteractionEvent[];
  recentDriveChanges?: DriveChangeEvent[];
}
...
async function getJson<T>(path: string): Promise<T> {
  ...
  if (!response.isSuccessful()) throw new Error(`心潮服务返回 HTTP ${response.statusCode}`);
  return JSON.parse(String(response.content ?? "{}")) as T;
}

function activeSessionId(state: XinchaoState): string {
  ...
  if (overlays[0]?.sessionId) return String(overlays[0].sessionId);
  const events = [...(state.recentConversationEvents ?? [])].reverse();
  return String(events.find(event => event.sessionId)?.sessionId ?? "default");
}

function recentTrajectory(state: XinchaoState, sessionId: string, nowMs: number): TrajectoryPoint[] {
  ...
  return (state.recentConversationEvents ?? [])
    ...
    .map(event => ({
      ...
      types: (event.interactionTypes ?? [event.interactionType ?? ""])
        .map(String)
        .filter(type => Boolean(INTERACTION_LABELS[type]))
        .slice(0, 4),
    }))
    .filter(event => event.types.length > 0);
}

function aggregateRecentChanges(state: XinchaoState, nowMs: number): Record<string, number> {
  const cutoff = nowMs - 6 * 3_600_000;
...
export function dominantEmotion(snapshot: DashboardSnapshot): {
  key: string;
  label: string;
  color: string;
  value: number;
  ...
  const dimension = DIMENSIONS.find(item => item.key === selected.key);
  return {
    key: selected.key,
    label: dimension?.label ?? selected.key,
    color: EMOTION_COLORS[selected.key] ?? "#62F6FF",
...
export async function loadDashboardSnapshot(): Promise<DashboardSnapshot> {
  const [state, intentPayload] = await Promise.all([
    getJson<XinchaoState>("/v1/state"),
    getJson<{ intent?: IntentInfo | null; intents?: IntentInfo[] }>("/v1/intent"),
  ]);
