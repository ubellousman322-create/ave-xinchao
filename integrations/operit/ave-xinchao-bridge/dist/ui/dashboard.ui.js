...
function useValue(ctx, key, initial) {
    const state = ctx.useState(key, initial);
...
function signed(value) {
    const number = Number(value);
    if (!Number.isFinite(number) || Math.abs(number) < 0.0005)
        return "-";
...
function constellationPoints(snapshot, group) {
    const positions = [[48, 70], [112, 40], [182, 66], [250, 38], [302, 112]];
    return dashboard_data_1.DIMENSIONS
...
function emotionPoints(snapshot) {
    const positions = [[38, 45], [112, 32], [186, 57], [262, 35], [306, 93], [74, 108], [154, 112], [232, 116]];
    return EMOTION_KEYS.map((key, index) => {
        const dimension = dashboard_data_1.DIMENSIONS.find(item => item.key === key);
        return {
            key,
...
function starSvg(points, selectedKey) {
...
function emotionSvg(snapshot, selectedKey) {
...
function coreSvg(snapshot) {
    const emotion = (0, dashboard_data_1.dominantEmotion)(snapshot);
...
function emotionReadings(snapshot) {
...
function dashboardHtml(snapshot) {
    const dominant = (0, dashboard_data_1.dominantEmotion)(snapshot);
    ...
    const trajectoryLabels = trajectory.map(point => point.types.map(type => escapeHtml(dashboard_data_1.INTERACTION_LABELS[type] ?? type)).join(" + ")).filter(Boolean);
    const tabs = [
...
function DashboardScreen(ctx) {
    const snapshot = useValue(ctx, "dashboardSnapshot", null);
    ...
    const refresh = async () => {
        ...
        error.set("");
        try {
            snapshot.set(await (0, dashboard_data_1.loadDashboardSnapshot)());
        }
        catch (reason) {
