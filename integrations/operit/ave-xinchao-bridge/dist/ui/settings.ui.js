...
function field(ctx, label, value, onValueChange, options = {}) {
...
function Screen(ctx) {
    const initial = (0, settings_1.loadBridgeSettings)();
    ...
    const save = () => {
        const saved = (0, settings_1.saveBridgeSettings)({
        ...
        status.set(complete ? "已保存，正在同步给心潮后台模型。" : "已保存，但接口地址、API Key 或模型名尚未填完整。");
        void (0, runtime_model_sync_js_1.syncRuntimeModelConfig)(true).then((result) => {
            status.set(result.configured ? `已同步：${result.model ?? "模型"}，互动分类、梦境与后台念头共用此配置。` : "已关闭心潮后台模型。");
        }).catch(() => status.set("已保存，但心潮后台模型同步失败；下轮对话会自动重试。"));
        void ctx.showToast("ave 心潮模型设置已保存");
    };
    return ctx.UI.LazyColumn({ fillMaxSize: true, padding: 16, spacing: 14 }, [
        ctx.UI.Text({ text: "ave 心潮桥接", style: "headlineSmall", fontWeight: "bold" }),
        ctx.UI.Text({
            text: "这里填写心潮共用模型。API Key 使用密码框，并保存在 Operit 包内私有配置中；互动分类、梦境、主动念头与白天浮现共用此配置，不会写入聊天、仓库或心潮状态。",
            style: "bodyMedium",
            color: "onSurfaceVariant",
        }),
        ctx.UI.Surface({ fillMaxWidth: true, shape: { cornerRadius: 10 }, containerColor: "surfaceVariant", alpha: 0.42 }, [ctx.UI.Row({ fillMaxWidth: true, padding: 14, verticalAlignment: "center", horizontalArrangement: "spaceBetween" }, [
                ctx.UI.Column({ weight: 1, spacing: 3 }, [
                    ctx.UI.Text({ text: "启用心潮共用模型", style: "titleMedium" }),
