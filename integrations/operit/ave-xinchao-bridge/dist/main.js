...
exports.sanitizeInteractionTags = sanitizeInteractionTags;
exports.flashThoughtsFromTags = flashThoughtsFromTags;
exports.stableFingerprint = stableFingerprint;
exports.extractPreviousCompletedTurn = extractPreviousCompletedTurn;
exports.onChatMessagePersisted = onChatMessagePersisted;
exports.registerToolPkg = registerToolPkg;
exports.onPromptFinalize = onPromptFinalize;
const settings_ui_js_1 = __importDefault(require("./ui/settings.ui.js"));
const dashboard_ui_js_1 = __importDefault(require("./ui/dashboard.ui.js"));
const xinchao_service_js_1 = require("./xinchao-service.js");
...
function sanitizeInteractionTags(value) {
    const source = Array.isArray(value)
        ? value
        : (typeof value === "object" && value !== null && Array.isArray(value.interactions)
            ? value.interactions
            : []);
...
function normalizedSender(value) {
    ...
    if (["user", "human", "me", "owner"].includes(sender))
        return "user";
    if (["assistant", "ai", "model", "bot"].includes(sender))
        return "assistant";
    return "other";
}
function flashThoughtsFromTags(tags) {
    const byKey = new Map();
    for (const tag of tags) {
        ...
        if (!previous || candidate.intensity > previous.intensity)
            byKey.set(candidate.key, candidate);
    }
    return [...byKey.values()].slice(0, 4);
}
function sessionChangesFromTags(tags) {
    ...
    return {
        ...
        sessionDeltas: Object.fromEntries(Object.entries(deltas).map(([key, value]) => [
            key,
...
function extractPreviousCompletedTurn(history, chatId) {
...
async function getJson(url, token, timeoutMs) {
    await (0, xinchao_service_js_1.ensureXinchaoService)();
...
async function classifyTurn(turn) {
    ...
    if (!(0, settings_js_1.classifierConfigured)(settings))
        return [];
    try {
        await (0, runtime_model_sync_js_1.syncRuntimeModelConfig)();
    }
    catch (error) {
    ...
    const payload = await postJson(`${settings.classifierBaseUrl}/chat/completions`, settings.classifierApiKey, requestBody, settings.classifierTimeoutMs);
    const parsed = parseJsonObject(payload.choices?.[0]?.message?.content);
    return sanitizeInteractionTags(parsed);
}
async function loadCurrentContext(chatId) {
    const query = `?session_id=${encodeURIComponent(chatId || "default")}&mode=turn&max_tokens=900`;
    const payload = await getJson(`${XINCHAO_BASE_URL}/v1/context${query}`, XINCHAO_SERVICE_TOKEN, 2500);
...
async function settleCompletedTurn(chatId, turn) {
    if ((0, settings_js_1.turnWasProcessed)(turn.fingerprint))
        return;
    const tags = await classifyTurn(turn);
    const session = sessionChangesFromTags(tags);
    const result = await postJson(`${XINCHAO_BASE_URL}/v1/conversation-event`, XINCHAO_SERVICE_TOKEN, {
    ...
    (0, settings_js_1.markTurnProcessed)(turn.fingerprint);
...
async function recordCurrentUserHeartbeat(chatId, fingerprint) {
    await postJson(`${XINCHAO_BASE_URL}/v1/conversation-event`, XINCHAO_SERVICE_TOKEN, { event_id: `operit-input-${fingerprint}`, session_id: chatId }, 3500);
}
async function onChatMessagePersisted(event) {
    ...
    if (!content || content.includes(INJECTION_OPEN))
        return;
...
function registerToolPkg() {
    void (0, xinchao_service_js_1.ensureXinchaoService)()
        .then(() => (0, runtime_model_sync_js_1.syncRuntimeModelConfig)(true))
    ...
    ToolPkg.registerToolboxUiModule({
        ...
        runtime: "compose_dsl",
        screen: settings_ui_js_1.default,
        params: {},
    ...
    ToolPkg.registerAppLifecycleHook({
        ...
        function: xinchao_service_js_1.onXinchaoLifecycle,
    });
    ToolPkg.registerAppLifecycleHook({
...
async function onPromptFinalize(event) {
    ...
    if (!processedInput.trim() || processedInput.includes(INJECTION_OPEN))
        return null;
    try {
