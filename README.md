...
- **每轮即时互动＋15 分钟轨迹**：互动标签每轮立即作用于累计状态；Context 另按 `session_id` 保留最近 15 分钟、最多 8 轮的标签顺序，不保存聊天正文。
- **Operit 桥接**：`integrations/operit/ave-xinchao-bridge` 提供私有模型配置、上一轮提取、幂等回传与每轮 Context 注入。
